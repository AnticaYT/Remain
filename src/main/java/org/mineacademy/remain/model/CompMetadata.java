...
public class CompMetadata {
	...
	public static final void setMetadata(Entity entity, String key, String value) {
		...
		try {
			...
			// Lost upon reload
			entity.setMetadata(key, new FixedMetadataValue(Remain.getPlugin(), tag));
		}
	}

	// Format the syntax of stored tags
	private static final String format(String key, String value) {
		return Remain.getPlugin().getName() + DELIMITER + key + DELIMITER + value;
	}
	...
	private static final String getTag(String raw, String key) {
		...
		return parts.length == 3 && parts[0].equals(Remain.getPlugin().getName()) && parts[1].equals(key) ? parts[2] : null;
	}
	...
	private static final boolean hasTag(String raw, String tag) {
		...
		return parts.length == 3 && parts[0].equals(Remain.getPlugin().getName()) && parts[1].equals(tag);
	}
}
